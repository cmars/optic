...
Setting up your tests to run with Optic locally gives you the ability to fail your tests when your schema is not met, without adding any additional tests. There are flags to fail tests and pass through exit codes which are particularly helpful in CI environments. Learn more about [runing in CI](/document/test/run-in-ci) so you can have the same benefits of schema checks with your current tests throughout your development cycle.