...
<ExpandDocs title="1. Potential error: the Optic port is occupied" defaultOpen={false}>

The first error that may arise is if Optic cannot listen on the inbound port. This is most commonly caused by the API project already running on the port.
...
<ExpandDocs title="2. Potential error: the project start command is not working" defaultOpen={false}>

Another common error is if the start task cannot run. The output of the command, and any failure items, will show up here. In this example the command `noder` is not found. The running command was `noder server.js --port $PORT` instead of `node ...`. This looks like a simple typo, a quick fix.
...
<ExpandDocs title="3. Potential error: the project does not start on the correct port" defaultOpen={false}>
...
Congratulations! Once you're done, you can check out the Optic dashboard to see your traffic and [document your test endpoints](/document). This will help you inspect the request and response bodies and make sure your integration is operating correctly.
