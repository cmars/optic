...
- `optic.yml` - Optic's config file. Here you can set up various methods of capturing API Traffic, that can be shared with your team, and called upon in CI/CD.
  ...
  - `enviroments` - deployed versions of your API that you can intercept traffic to from the client side. ie `api intercept production --chrome` will open your production webapp and collect all the traffic to your API that is seen in the Chrome Network tab.
