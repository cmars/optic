...
- `--transparent-proxy` A deprecated feature, use [api intercept](/document) instead where possible. Configures Optic to transparently forward all traffic, which may be necessary for certain application configurations. 
