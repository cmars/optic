...
### Capture traffic to document

Every API documentation session will start with capturing some traffic. We just finished capturing some traffic while testing our Optic integration with your project, which will be a good start to understanding how your API behaves today. At any time, you can start a new session and capture more traffic with:

`api run start`

Optic will continue to capture traffic until it is stopped with `ctrl+c` on the terminal.
...
- Run `api status`
  - If you see API paths, run `api status --review` to open the Optic UI
