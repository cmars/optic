...
By default, Optic lets you [add one endpoint at a time](document/baseline). When adding each endpoint individually, you get a preview of what Optic has learned about that endpoint from the traffic. The **bulk mode** switch allows you to add multiple endpoints at once. It learns the same things about each endpoint: the only difference is you don't have to go through each one individually, seeing what is learned. You can still name the endpoints in the right-hand column as you learn them in bulk, and you'll be able to see all of the learned endpoints under the **Documentation** tab once they have been saved.

<Paper elevation={3}>

![Enabling bulk mode](/img/docs/bulk-enabled.png)
...
The plus signs have been replaced by checkboxes - you check these checkboxes to indicate which routes you want to learn. You can learn regular endpoints and [parameterized endpoints](/document/parameters) in bulk mode. The only difference is, instead of clicking a + icon to go to a preview, you check the **checkbox** to mark it for learning
...
Don't worry about making a mistake. Path parameters can be added before learning an endpoint. Click on the path element that should be a parameter, and Optic will consolidate all of the traffic it sees that matches that path. Then you can add an ID, and the endpoint is ready to be added to your documentation. You don't even need to uncheck and recheck the endpoints: Optic keeps track of what you selected

<Paper elevation={3}>

![The first endpoint is staged](/img/docs/bulk-parameterize.png)

</Paper>

**Do this ⤵️**

- [parameterize](/document/parameters) an endpoint for which you have multiple paths checked.
- Notice the paths all consolidate down to the one parameterized endpoint.
- Add an ID

If you have already added and endpoint that should be parameterized, you can [discard the endpoint](/document/parameters#did-you-make-a-mistake) and learn it again, with or without bulk mode.
...
Your endpoints will be automatically learned in the same way they are learned individually, and added to the right-hand side of the documentation window. They have a green highlight, indicating they are ready to be saved to your documentation. Again, if you added and endpoint that should be parameterized, you can [discard the endpoint](/document/parameters#did-you-make-a-mistake) and learn it again, with or without bulk mode.
